...
def safe_run():
    """Безопасный запуск приложения с обработкой ошибок отображения"""
    try:
        dpg.create_context()
        ...
        dpg.create_viewport(title='Node Editor with Palette', resizable=True, width=1440, height=810)
        
        # Проверяем, можем ли мы инициализировать графический контекст
        try:
            dpg.setup_dearpygui()
            ...
            print(f"Ошибка инициализации графического интерфейса: {e}")
            print("Программа может работать только в среде с графическим интерфейсом")
            
    except Exception as e:
        print(f"Критическая ошибка при создании контекста: {e}")
    finally:
        try:
